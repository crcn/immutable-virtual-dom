...
ComponentHydrator.prototype.hydrate = function(root, bindings) {
  ...
  if (this.hasDynamicAttributes) {
    _hydrateDynamicAttributes(ref, this.options.attributes, this.dynamicAttributes, bindings);
  }
  bindings.push(ref);
};
...
ElementAttributeHydrator.prototype.hydrate = function(root, bindings) {
  if (!this._marker) this._marker = this.section.createMarker();
  _hydrateDynamicAttributes(this._marker.findNode(root), this.attributeClasses, this.dynamicAttributes, bindings);
};

/**
 */

function _hydrateDynamicAttributes(ref, attributeClasses, dynamicAttributes, bindings) {
  for (var key in dynamicAttributes) {
    var clazz = attributeClasses[key];
    var attr = new clazz(ref, key, dynamicAttributes[key]);
