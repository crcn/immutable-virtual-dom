...
Element.prototype.freeze = function(options) {

  var components = options.components || {};

  if (components[this.nodeName]) {
    return this.freezeComponent(components[this.nodeName], options);
  }
...
Element.prototype.freezeComponent = function(clazz, options) {
  var section = new FragmentSection(options.document);
  options.hydrators.push(new ComponentHydrator(clazz, section, fragment.apply(this, this.childNodes)));
...
Element.prototype.freezeElement = function(options) {
  ...
  for (var i = 0, n = this.childNodes.length; i < n; i++) {
    element.appendChild(this.childNodes[i].freeze(options));
  }
